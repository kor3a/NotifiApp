...
//
//  IMPORTANT — these declarations are mirrored verbatim from
//  "Geolocation_v1.0.0/Services/WatchSyncModels.swift". The watch target cannot
//  see the iOS target's sources, so the two copies must be edited together, the
//  same way WidgetStoreData is mirrored into the widget extension. Changing a
//  property name there without changing it here silently breaks decoding.
//
