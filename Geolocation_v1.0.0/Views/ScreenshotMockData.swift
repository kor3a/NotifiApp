...
/// Addresses the drawn portraits by URL, so a mock friendship can carry one
/// the way a real one carries an upload.
///
/// The drawing itself is `PortraitAvatar`, which the whole app uses for anyone
/// without a profile photo; this only maps `mock-avatar://` URLs onto it so
/// `ProfileImageCache` can hand the portrait back without a network call.
enum ScreenshotAvatarFactory {
    ...
    static func image(forMockURL urlString: String) -> UIImage? {
        ...
        return PortraitAvatar.image(variant: index)
    }
}
